...
$action->helper->route('action/createresume', function () {
    ...
    $action->onlyForAuthUser();
     ...
     $run=$action->db->insert('resume', 'user_id,name,headline,objective,contact,skills,experience,education,url',$resume_data);
     if($run){
...
$action->helper->route('action/deleteresume/$url', function ($data) {
    global $action;
     $url = $data['url'];
     $action->db->delete('resume',"url='$url'");
...
$action->helper->route('home', function(){
    ...
    $data['profile'] = 'active';
    $data['logout'] = 'active';
    $data['resume']= $action->db->read('resume','*',"WHERE user_id=".$action->user_id());
...
$action->helper->route('resume/$url', function($data){
    global $action;
   $resumedata= $action->db->read("resume","*","WHERE url='".$data['url']."'");
     if(!$resumedata){
         $action->helper->redirect('home');
     }
    ...
    $data['logout'] = 'active';
if($data['type']==1){
    $action->view->load('cv_content1',$data);
}else{
         $action->helper->redirect('home');
}
});
